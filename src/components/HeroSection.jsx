...
const INITIAL_LEADS = [
  { id: 1, name: 'Rahul Sharma', phone: '+91 9876543210', course: 'JEE Main & Advanced', status: 'New', date: '2026-06-01' },
  { id: 2, name: 'Priya Patel', phone: '+91 9876543211', course: 'NEET UG', status: 'Contacted', date: '2026-06-02' },
  { id: 3, name: 'Amit Kumar', phone: '+91 9876543212', course: '11th-12th Academics', status: 'Converted', date: '2026-05-28' },
  { id: 4, name: 'Neha Singh', phone: '+91 9876543213', course: 'CUET', status: 'Lost', date: '2026-05-30' },
  { id: 5, name: 'Vikram Gupta', phone: '+91 9876543214', course: 'CAT', status: 'New', date: '2026-06-04' },
];

const INITIAL_PAYMENTS = [
  { id: 101, studentName: 'Amit Kumar', course: '11th-12th Academics', totalFee: 45000, paid: 25000, due: 20000, dueDate: '2026-06-15', status: 'Pending' },
  { id: 102, studentName: 'Rohan Das', course: 'JEE Main & Advanced', totalFee: 85000, paid: 85000, due: 0, dueDate: '2026-05-10', status: 'Paid' },
  { id: 103, studentName: 'Sneha Roy', course: 'NEET UG', totalFee: 80000, paid: 40000, due: 40000, dueDate: '2026-06-02', status: 'Overdue' },
  { id: 104, studentName: 'Karan Verma', course: '6th-10th Foundation', totalFee: 30000, paid: 10000, due: 20000, dueDate: '2026-06-20', status: 'Pending' },
  { id: 105, studentName: 'Pooja Iyer', course: 'CLAT', totalFee: 50000, paid: 20000, due: 30000, dueDate: '2026-06-01', status: 'Overdue' },
];
...
const StatCard = ({ title, value, trend, icon: Icon, trendUp }) => (
  <Card className="p-6">
    ...
    <div className={`mt-4 flex items-center text-sm ${trendUp ? 'text-emerald-600' : 'text-rose-600'}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Total Active Leads" value={leads.length} trend="+12%" icon={Users} trendUp={true} />
        <StatCard title="Total Students" value="850" trend="+5%" icon={GraduationCap} trendUp={true} />
        <StatCard title="Monthly Revenue" value="₹1,40,000" trend="-2%" icon={CreditCard} trendUp={false} />
        <StatCard title="Pending Dues" value={`₹${totalDue.toLocaleString()}`} trend={`${overdueCount} Overdue`} icon={AlertCircle} trendUp={false} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-6">
          <h3 className="text-lg font-bold text-slate-900 mb-6">Enrollment by Category</h3>
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Track and manage student inquiries.</p>
        </div>
...
const PaymentsView = ({ payments, setPayments, showToast }) => {
  ...
  const filteredPayments = payments.filter(p => {
    const matchesSearch = p.studentName.toLowerCase().includes(searchTerm.toLowerCase()) || p.course.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === 'All' ? true : p.status === statusFilter;
    return matchesSearch && matchesStatus;
  });

  const handleRemind = (studentName, emailSimulated) => {
    showToast('Reminder Sent', `Payment reminder email sent to ${studentName}.`, 'success');
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-900">Fee & Payment Status</h2>
        <p className="text-slate-500 text-sm">Manage student fees and send reminders.</p>
      </div>
              ...
              {filteredPayments.map(payment => {
                ...
                return (
                  <tr key={payment.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium">₹{payment.totalFee.toLocaleString()}</td>
                    <td className="px-6 py-4 text-emerald-600">₹{payment.paid.toLocaleString()}</td>
                    <td className="px-6 py-4 text-rose-600 font-medium">₹{payment.due.toLocaleString()}</td>
                    <td className="px-6 py-4">
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-slate-500 text-sm">Manage your academy's preferences and automations.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
              <div>
                <p className="font-medium text-slate-900 text-sm">5-Day Pre-Due Email</p>
                <p className="text-xs text-slate-500">Automatically send email 5 days before due date</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" className="sr-only peer" defaultChecked onChange={() => showToast('Settings Updated', 'Automation preferences saved.')} />
                <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
              </label>
            </div>
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
              <div>
                <p className="font-medium text-slate-900 text-sm">Overdue SMS Alerts</p>
                <p className="text-xs text-slate-500">Send SMS on the day payment is overdue</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" className="sr-only peer" onChange={() => showToast('Settings Updated', 'Automation preferences saved.')} />
                <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
              </label>
            </div>
          </div>
        ...
        <Card className="p-6">
          ...
          <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); showToast('Success', 'Profile settings updated.'); }}>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Academy Name</label>
              <input type="text" defaultValue="GSM Academy" className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Support Email (Sender)</label>
              <input type="email" defaultValue="support@gsmacademy.in" className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            <div className="pt-2">
  ...
  return (
    <div className="h-screen bg-slate-50 flex font-sans text-slate-900">
      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2">
        {toasts.map(toast => (
          <div key={toast.id} className="bg-slate-800 text-white px-4 py-3 rounded-lg shadow-lg flex items-center gap-3 animate-fade-in-up">
      ...
      `}>
        <div className="p-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">ERP & CRM Portal</p>
          </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 sm:px-6 z-10 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center text-sm text-slate-500">
              <span className="font-medium text-slate-900 capitalize">{activeTab.replace('-', ' ')}</span>
        ...
        <div className="flex-1 overflow-auto p-4 sm:p-6 lg:p-8">
          <div className="max-w-7xl mx-auto">
            {activeTab === 'dashboard' && <DashboardView leads={leads} payments={payments} />}
            {activeTab === 'leads' && <LeadsView leads={leads} setLeads={setLeads} showToast={showToast} />}
            {activeTab === 'payments' && <PaymentsView payments={payments} setPayments={setPayments} showToast={showToast} />}
            ...
            {(activeTab === 'courses') && (
              <div className="flex flex-col items-center justify-center h-64 text-center">
                ...
                <p className="text-slate-500 max-w-sm">
                  The {activeTab} module is currently being built and will be available in the next system update.
                </p>
              </div>
            )}
          </div>
